...
public class binaryNet
{
    ...
    private int inputNodes[]; // inputs to the network
    private int outputNodes[]; // outputs from the network
    private int theWeights[][]; // weights matrix
    private int theNeurons[]; // the neurons
    ...
    // constructor
    public binaryNet(Dataset ds, double thold)
    {
		theData = ds;
		...
		inputNodes = new int[numBits];
		outputNodes = new int [numBits];
		theNeurons = new int[numBits];
		theWeights = new int[numBits][numBits];
	...
	public void feedForward()
	{
		...
		for (int m = 0; m < numBits; m++)
		{
			...
			// thresholding
			if (theNeurons[m] > threshold)
			{
				theNeurons[m] = 1; // the neuron fires
			}
			else
			{
				theNeurons[m] = -1;
	...
	{		
		...
		for (int r = 0; r < rows; r++)
		{
			...
			{
				theData[r][c] = outputNodes[(r * cols) + c];
			}
		}
